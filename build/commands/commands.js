...
export const setupCommands = (bot) => {
    // Greeting command
    bot.command("test1", (ctx) => ctx.reply("this uses context reply"));
    // Lambda command
    bot.command("test2", Telegraf.reply("this uses telegraph's reply"));
    // Lambda command
    bot.command("image", (ctx) => ctx.replyWithPhoto("https://shapes.inc/api/public/avatar/emuotori-n6m4", {
        caption: "This is EMU OTORI!!",
    }));
    bot.command("image2", (ctx) => {
    ...
    */
    ...
    bot.on("photo", async (ctx) => {
        try {
            ...
            if (caption.includes("/postimage")) {
                ...
                ctx.reply(fileId);
                ctx.telegram.getFileLink(fileId).then((link) => {
                    https.get(link, (response) => response.pipe(fs.createWriteStream(path)));
                });
                const pdsUrl = "https://bsky.social/xrpc/com.atproto.repo.uploadBlob";
                const imgBytes = fs.readFileSync(path);
                const response = await axios.post(pdsUrl, imgBytes, {
                    headers: {
    ...
    bot.command("post", async (ctx) => {
        try {
            ...
            if (!text) {
                await ctx.reply("Please provide some text for the post. Usage: /post [your text]");
                return;
            }
            await BskyConnection.agent.post({
                text: text,
            ...
            await ctx.reply("Sorry, there was an error posting to Bluesky. Please try again later.");
        }
    });
    // Help command
